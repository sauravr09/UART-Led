...
void USART_Init (USART_TypeDef * USARTx) {
  ...
  if (USARTx == UART4){	
	...
	if (USARTx == USART2){
		...
		USART2->CR1 &= ~USART_CR1_IDLEIE;  			// Idle Line Detected Interrupt
...
uint8_t USART_READ_Nonblocking(USART_TypeDef * USARTx){
	...
	if (!(USARTx->ISR & USART_ISR_RXNE)){ // if there is nothing to read
		return 0;
	}
	return ((uint8_t)(USARTx->RDR & 0xFF)); // read from the buffer receiver.
}
...
//	uint8_t one_char;
//
//	one_char = ((uint8_t)(USART2->RDR & 0xFF));
//
//	// call a function which will process the user input
//	processInput(one_char);
//
//}
